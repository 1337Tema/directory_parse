...
TEXT_FILE_EXTENSIONS = {
    '.txt', '.md', '.py', '.json', '.xml', '.html', '.htm', '.css', '.js', '.c', '.cpp', '.h', '.hpp',
    '.java', '.cs', '.sh', '.bash', '.rb', '.php', '.go', '.rs', '.swift', '.kt', '.kts', '.yaml', '.yml',
    '.ini', '.cfg', '.conf', '.log', '.sql', '.csv', '.tsv', '.rtf', '.tex', '.sub', '.srt', '.ps1',
    '.bat', '.cmd', '.dockerfile', '.gitignore', '.gitattributes', '.env', '.properties', '.toml',
    # Add more as needed
}
...
def _write_folder_tree_recursive(
    ...
    Optionally includes text content of files based on their extension.
    ...
    include_file_content: Boolean, whether to read and include content of text files.
    """
    try:
                ...
                if file_ext.lower() in TEXT_FILE_EXTENSIONS:
                    try:
                        with open(current_path, 'r', encoding='utf-8', errors='replace') as f_content:
                            output_file.write(f"{full_content_prefix}--- File Content (text, up to {MAX_FILE_CONTENT_LINES} lines) ---\n")
                            lines_written = 0
                        ...
                        output_file.write(f"{full_content_prefix}[Text file with unreadable encoding - Content not displayed]\n")
                    except IOError as e:
                    ...
                    # Not a recognized text file extension
                    output_file.write(f"{full_content_prefix}[Non-text file or unrecognized extension - Content not displayed]\n")
        else:
...
def _generate_and_save_single_structure_file(
    ...
    """
    Generates the directory structure (and optionally text file content)
    ...
    include_file_content_flag: Boolean, whether to include content of text files.
    """
...
if __name__ == "__main__":
    ...
    parse_with_content_input = input(
        "Include text file content? (Uses a list of common text extensions. This will create an additional file if 'yes') (yes/no) [no]: "
    ).lower()
